...
class HistoryViewProvider {
    constructor(_extensionUri, _gitService) {
        this._extensionUri = _extensionUri;
        this._gitService = _gitService;
        this._currentCommits = [];
    }
    ...
    showHistory(commits, repoPath, filePath, startLine, endLine) {
    ...
    navigate(direction) {
        if (!this._currentHash || this._currentCommits.length === 0)
            return;
        ...
        if (newIndex < 0)
            newIndex = 0;
        if (newIndex >= this._currentCommits.length)
            newIndex = this._currentCommits.length - 1;
    ...
    async _openRevision(hash) {
        if (!this._repoPath || !this._filePath)
            return;
        try {
            ...
            const uri = vscode.Uri.parse(`${gitContentProvider_1.GitContentProvider.scheme}:${displayPath}?revision=${hash}&rootPath=${this._repoPath}&originalPath=${this._filePath}`);
            ...
            if (this._startLine && this._endLine) {
                const range = new vscode.Range(Math.max(0, this._startLine - 1), 0, Math.max(0, this._endLine - 1), 1000);
...
HistoryViewProvider.viewId = 'git-intellij.historyView';
//# sourceMappingURL=historyWebview.js.map